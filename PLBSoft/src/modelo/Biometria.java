package modelo;

import java.io.Serializable;
import java.math.BigDecimal;

import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.GeneratedValue;
import javax.persistence.GenerationType;
import javax.persistence.Id;
import javax.persistence.ManyToOne;
import javax.persistence.Table;

@Table(name = "BIOMETRIA")
@Entity
public class Biometria implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 4402905542933358011L;

	@Id
	@GeneratedValue(strategy = GenerationType.IDENTITY)
	private Long id;
	
	@Column
	private Boolean status;
	
	@Column
	private BigDecimal tamanhoMedio;
	
	@Column
	private BigDecimal pesoMedio;
	
	@Column
	private BigDecimal quantidadeAmostra;
	
	@ManyToOne
	private Lote loteId;

	public Long getId() {
		return id;
	}

	public void setId(Long id) {
		this.id = id;
	}

	public Boolean getStatus() {
		return status;
	}

	public void setStatus(Boolean status) {
		this.status = status;
	}

	public BigDecimal getTamanhoMedio() {
		return tamanhoMedio;
	}

	public void setTamanhoMedio(BigDecimal tamanhoMedio) {
		this.tamanhoMedio = tamanhoMedio;
	}

	public BigDecimal getPesoMedio() {
		return pesoMedio;
	}

	public void setPesoMedio(BigDecimal pesoMedio) {
		this.pesoMedio = pesoMedio;
	}

	public BigDecimal getQuantidadeAmostra() {
		return quantidadeAmostra;
	}

	public void setQuantidadeAmostra(BigDecimal quantidadeAmostra) {
		this.quantidadeAmostra = quantidadeAmostra;
	}

	public Lote getLoteId() {
		return loteId;
	}

	public void setLoteId(Lote loteId) {
		this.loteId = loteId;
	}

	@Override
	public int hashCode() {
		final int prime = 31;
		int result = 1;
		result = prime * result + ((id == null) ? 0 : id.hashCode());
		return result;
	}

	@Override
	public boolean equals(Object obj) {
		if (this == obj)
			return true;
		if (obj == null)
			return false;
		if (getClass() != obj.getClass())
			return false;
		Biometria other = (Biometria) obj;
		if (id == null) {
			if (other.id != null)
				return false;
		} else if (!id.equals(other.id))
			return false;
		return true;
	}
	
}
